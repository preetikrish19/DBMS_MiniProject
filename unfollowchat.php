...
include('db.php');
$i=1;
$sql = "SELECT DISTINCT enrolldetails.uid, enrolldetails.name, enrolldetails.year, enrolldetails.email FROM enrolldetails INNER JOIN chat ON chat.student_id=enrolldetails.uid  WHERE enrolldetails.uid NOT IN( SELECT DISTINCT f.uid FROM follow f) AND chat.mentor_id=$_SESSION[mid]";
