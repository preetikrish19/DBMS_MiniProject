<?php 
    ...
    if(isset($_SESSION['login'])){
        ...
        $qid = $_POST['qid']; 
        //echo $qid;
        $sql2 = "SELECT * FROM linkagementor WHERE qid=$qid ORDER BY likes DESC";
        ...
        //if($type == 1){
            foreach($details2 as $detail2){
                echo "
                <div class='ans'>
                <b><h4 style='color: white'>$detail2[name]</h4></b>
                <ul>
                <li>
                <div>
                <p style='color: white'>$detail2[answer]</p>
                </div>
                </li>
                <li>
                <div style='padding: 2px; margin-top: 5px;'>";
                        $results = mysqli_query($con, "SELECT * FROM answerlikes WHERE user_id=$_SESSION[uid] AND ans_id=$detail2[ans_id] AND login_type=$_SESSION[login]");
                        if (mysqli_num_rows($results) == 1 ):
                            echo "
                            <span class='unlike fa fa-thumbs-up' data-id='$detail2[ans_id]'></span> 
                            <span class='like hide fa fa-thumbs-o-up' data-id='$detail2[ans_id]'></span>"; 
                        else:
                            echo "
                            <span class='like fa fa-thumbs-o-up' data-id='$detail2[ans_id]'></span> 
                            <span class='unlike hide fa fa-thumbs-up' data-id='$detail2[ans_id]'></span>"; 
                        endif;
                            echo "<span class='likes_count'>$detail2[likes] likes</span>";
                echo"
                    </div>
                </li>
                </ul>
    ...
    elseif ($_SESSION['login']==1){
        ...
        //if($type == 1){
            foreach($details2 as $detail2){
                ...
                <li>
                <div style='padding: 2px; margin-top: 5px;'>";
                        $results = mysqli_query($con, "SELECT * FROM answerlikes WHERE user_id=$_SESSION[mid] AND ans_id=$detail2[ans_id] AND login_type=$_SESSION[login]");
                        if (mysqli_num_rows($results) == 1 ):
                            echo "
                            <span class='unlike fa fa-thumbs-up' data-id='$detail2[ans_id]'></span> 
                            <span class='like hide fa fa-thumbs-o-up' data-id='$detail2[ans_id]'></span>"; 
